...
from hidrografia_django.models import Cauces as CaucesModel
...
class Cauces:
    ...
    def insert(self, d:dict):
        ...
        query=""" 
            select id from hidrografia_django_cauces where ST_relate(
                geom,
                %s,
                'T********'
            )
        ...
        return {'ok': True, 'message':'Cauce inserted', 'data': [d]}
    ...
    def selectone(self, d:dict):
        ...
        f = CaucesModel.objects.filter(id=d['id'])
        ...
        if len(l) == 0:
            return {'ok': False, 'message': f"No existen cauces con id {d['id']}", 'data': None}
    ...
    def selectall(self):
        ...
        return {'ok': True, 'message': f"Todos los cauces: {len(data)}", 'data': data}
    ...
    def update(self, d:dict):
        ...
        if len(r)>0:
            return {'ok': False, 'message': 'La geometría intersecta', 'data': r}
        ...
        if len(l)>0:
            ...
            return {'ok': False, 'message': f"No cauces found with id {d['id']}", 'data': None}

        b.geom=g
        b.nombre=d['nombre']
        b.tipo=d['tipo']
        b.longitud_km=d["longitud_km"]
        b.caudal_medio=d["caudal_medio"]
        b.estado_ecologico=d['estado_ecologico']
        b.save()
        d=model_to_dict(b)
        d['geom']=g.wkt
        d['data_creation']=d['data_creation'].strftime("%Y-%m-%d %H:%M:%S")

        return {'ok':True, 'Message': f"Updated cauces: {len(l)}",
                'data':[d]}
    ...
    def delete(self, d:dict):
        ...
        l=list(f)
        if len(l)<1:
            return {"ok":False, "Message": f"No cauces with the id {d['id']}", "data":None}
        b=l[0]
        b.delete()
        return {'ok':True, 'Message': f"Cauces deleted: 1",
                'data':[{'id':d["id"]}]}
